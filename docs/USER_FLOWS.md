...
│   │   └── ❸ Đặt hàng  →  /order?consultation=<id>
...
```
  ...
  └──────────┬────────────────────────────────┘                            │
             │ ⚠️ hasPrinting → "Đặt hàng ngay" KHÓA tới khi có mockup      │
             │   (trừ khi service chưa cấu hình → status unavailable)       │
             │ "Đặt hàng ngay" → /order?consultation=<id>                  │
             ├─────────────────────────────►                               │
             ▼                                                             │
  ┌───────────────────────────────┐                                       │
  │  ❸ /order — cần login submit  │                                       │
  │  • Nhiều sản phẩm / 1 đơn     │                                       │
  │    (thêm · xóa dòng)          │                                       │
  │  • Mỗi dòng: sản phẩm · SL ·  │                                       │
  │    D×R×C · số lớp · cần in? ·  │                                      │
  │    ghi chú                    │                                       │
  │  • Liên hệ: Họ tên · SĐT (đủ  │                                       │
  │    10 số) · Email · địa chỉ ·  │                                      │
  │    ghi chú đơn                │                                       │
  │  • Phương thức: chỉ "Giao     │                                       │
  │    hàng"                      │                                       │
  │  • Thiết kế: PNG/JPG/PDF/AI/  │                                       │
  │    EPS ≤10MB                  │                                       │
  │  • Thanh toán: COD / CK       │                                       │
  │  • Banner handoff in ấn (nếu   │                                      │
  │    có mockup + khuôn bế)      │                                       │
  │  • Tổng TẠM TÍNH — server     │                                       │
  │    tính lại, staff duyệt final │                                      │
  └──────────┬────────────────────┘                                       │
             │  POST /api/orders                                           │
             ├─────────────────────────────►                               │
...
`preferred_layers`, `flute_type`, `purchase_frequency`. Liên hệ + địa chỉ thuộc
❸ `/order`; logo + vị trí in thuộc panel Mockup ở ❷. Columns
...
### 2.3 Payload thật của `POST /api/orders`

```jsonc
{
  "consultationId": "<uuid>|omitted",
  "items": [{
    "productId": "<uuid>",
    "quantity": 500,
    "dimensions": { "length": 30, "width": 20, "height": 15, "layers": 3, "boxStyleId": "rsc_a1" },
    "printingSpecs": { "mockupUrl": "…", "dielineUrl": "…", "printPositionLabel": "…" },
    "notes": "…"
  }],
  "paymentMethod": "cod" | "bank_transfer",
  "contactName": "Nguyễn Văn A",
  "contactPhone": "0901234567",       // regex ^\d{10}$
  "contactEmail": "a@example.com",
  "deliveryMethod": "delivery",        // literal — 'pickup' không được accept
  "deliveryAddress": "…",
  "notes": "…"
}
```

Server **không** nhận giá từ client: `unit_price` lấy từ `products.base_price`,
`total_amount` = Σ subtotal, `deposit_amount` tính theo `pricing.ts`.
`customerId` chỉ staff được truyền; `order_items.product_name/code` là snapshot
từ DB. Nếu insert `order_items` fail → order vừa tạo bị **delete** để rollback.
...
> `print_faces` — sau đó handoff sang `/order` mang ảnh của người vừa ghi đè.
> Fix gọn: lấy `user.id` trong route và reuse đúng guard của
> `src/lib/mockup/handoff.ts` (`if (c.customer_id && c.customer_id !== user.id) → 403`).
>
...
khi đặt hàng: `src/lib/mockup/handoff.ts` cho phép lấy handoff của tư vấn ẩn
danh (guard là `if (consultation.customer_id && consultation.customer_id !== user.id) return null`),
nên một deep-link tư vấn anonymously tạo ra vẫn sinh đơn có `customer_id` của
người mở link.
...
> kể cả của khách khác. `src/lib/mockup/handoff.ts` có guard
> (`consultation.customer_id !== user.id → null`) nhưng route result thì không.
...
| Nav doc cũ vẽ ở chân trang | Nav thật nằm ở TRÊN, do `src/components/layout/DashboardNav.tsx` render trong `src/app/(auth)/layout.tsx`: `Tổng quan · Đơn hàng · Lịch sử · Hồ sơ` + nút Đăng xuất (`signOut()` → `/`). History + Profile KHÔNG orphan |  
...
Hai payment method: `cod` và `bank_transfer` — **chọn ở form đặt hàng / reorder**,
mặc định `cod`. Không có gate nào bắt login để *chọn*, nhưng submit order thì bắt buộc.

```
  CUSTOMER                              SYSTEM
     │ chọn "Chuyển khoản" ở /order        │
...
| **Clear CTAs** | ✅ mỗi màn 1 primary: `Yêu cầu AI tư vấn` → `Tạo ảnh mockup` → `Đặt hàng ngay` → `Gửi đơn hàng` |
...
| **Mobile-first** | ✅ consultation `max-w-6xl` 1 cột → `lg:grid-cols-[1.2fr_1fr]`; order `lg:grid-cols-[minmax(0,1fr)_24rem]`; timeline scroll ngang (`min-w-[760px]`) |
...
| ✅ P0 xong | `/order` | multi-item, handoff in ấn, tự mở modal thanh toán |
| ✅ P0 xong | `/dashboard` · `/dashboard/orders` · `/orders/[id]` · `/history` · `/reorder` · `/profile` | RSC + URL-state modals (`?modal=create`, `?orderId=<id>`) |
