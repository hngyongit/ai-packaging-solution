...
export default function StaffProfilePage() {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      if (u) {
        const { data: profile } = await supabase
          .from('profiles')
          .select('full_name, phone, company_name, default_address, role')
          .eq('id', u.id)
          .maybeSingle()
  ...
  const roleLabel = role === 'admin' ? 'Quản trị viên' : 'Nhân viên kinh doanh'
    ...
    <div className="max-w-2xl space-y-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Envelope className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
            <Input
              id="email"
              value={user?.email || ''}
              readOnly
              className="pl-9 bg-gray-100 text-gray-500"
            />
          </div>
